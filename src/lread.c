...
#define Ungetc(x, y) (*l_ungetc)(x, y)
    ...
    static int
#ifdef KR_headers
        l_R(poststar, reqint) int poststar,
        reqint;
#else
...
c_le(cilist *a)
#endif
    {
        ...
        if (!f__curunit->ufmt)
            err(a->cierr, 103, "lio") return (0);
    }

    int
#ifdef KR_headers
        l_read(number, ptr, len, type) ftnint *number,
        type;
    char *ptr;
    ftnlen len;
#else
...
#define Ptr ((flex *)ptr)
        ...
        for (i = 0; i < *number; i++)
        {
            if (f__lquit)
                return (0);
            if (l_eof)
                err(f__elist->ciend, EOF, "list in") if (f__lcount == 0)
                {
                    f__ltype = 0;
                    for (;;)
                    {
                        ...
                        {
                        case EOF:
                            err(f__elist->ciend, (EOF), "list in") case ' ' : case '\t' : case '\n' : continue;
                        case '/':
                            f__lquit = 1;
                            goto loopend;
                        case ',':
